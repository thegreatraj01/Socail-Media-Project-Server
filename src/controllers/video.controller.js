...
const getAllVideos = asyncHandler(async (req, res) => {
  ...
  if (isNaN(pageNumber) || pageNumber < 1) {
    throw new ApiError(
      ...
      "Invalid 'page' value. Must be a number ≥ 1."
    );
  }
  ...
  if (isNaN(pageSize) || pageSize < 1) {
    throw new ApiError(
      HTTP_STATUS_CODES.BAD_REQUEST.code,
      "Invalid 'limit' value. Must be a number ≥ 1."
    );
  }
  ...
  if (userId) {
    if (!isValidObjectId(userId)) {
      throw new ApiError(
        HTTP_STATUS_CODES.BAD_REQUEST.code,
        "Invalid 'userId'. Must be a valid MongoDB ObjectId."
      );
    }
    filter.owner = userId;
  }
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  if (!(newThumbnail || title || description)) {
    throw new ApiError(
      HTTP_STATUS_CODES.BAD_REQUEST.code,
      "give any feild to update thumbnail , title or description"
    );
  }
  ...
  if (video.owner.toString() !== req.user._id.toString()) {
    throw new ApiError(
      ...
      "You are not authorized to modify video details "
    );
  }
...
const togglePublishStatus = asyncHandler(async (req, res) => {
  ...
  if (video.owner.toString() !== req.user._id.toString()) {
    throw new ApiError(
      HTTP_STATUS_CODES.FORBIDDEN.code,
      "You are not authorized to modify publish status this video"
    );
  }
