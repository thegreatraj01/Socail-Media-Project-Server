    ...
    email: {
      ...
      validate: {
        validator: function (v) {
          ...
          return true; // Skip validation if no email is provided
        },
    ...
    mobileNumber: {
      ...
      validate: {
        validator: function (v) {
          ...
          if (v) {
            // Validate for a 10-digit mobile number (adjust regex for specific country formats if needed)
            return /^\d{10}$/.test(v);
          }
          return true; // Skip validation if no mobile number is provided
        },
        message: (props) =>
          `${props.value} is not a valid mobile number! It must be 10 digits.`,
      },
    },
